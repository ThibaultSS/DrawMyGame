...
6. `/play/{drawing}` replays a saved drawing (published, or unpublished-but-yours; anything else 404s). A drawing saved with its game settings fills the whole session — colours plus `gameSpeed`/`jumpHeight` — and goes straight to `/game`; a pre-settings drawing sets only `uploadedLevel` and detours through `/game-setting`
7. `/draw` (`DrawnLevelController`) is the second way in: draw the level on a canvas in the browser. The palette is a controller constant passed to the page as a prop, so `POST /draw-level` stores the PNG and writes those same colours into the session — no eyedropper — and redirects to `/game`
...
- Saving a level you *played* rather than uploaded (`/play` points the session at someone else's file) copies the file first, so each drawing owns its image. Deleting a drawing soft-deletes the row and removes the file; the `whereKeyNot` guard in `destroy()` is what stops the just-trashed row from matching itself.
- `levels:prune` (scheduled daily) deletes level images older than 24h that no drawing references — uploading needs no account and most uploads are never saved, so without it they accumulate forever. `POST /upload-level` and `POST /register` are throttled.
