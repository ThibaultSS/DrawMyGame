...
- The Draw page's palette is a **default, not a fixed set**: each colour can be changed and whatever is chosen is posted to `/start-game`, which already validates any `#rrggbb`. Two things make that safe. `colorsTooClose()` in `roleCheck.js` reads `DETECTION.colorTolerance` rather than repeating the number, so "far enough apart" means exactly what it means to the detector — and it checks each colour against the paper white too, or the page itself becomes a shape. And changing a colour **repaints what is already drawn in it**, within that same tolerance so anti-aliased edges come along, while leaving pixels near the paper alone so the repaint cannot creep outwards. Without it, earlier strokes keep the old colour and silently stop being platforms.
- `roleCheck.js` answers "will this picture parse?" by running `detectShapes` rather than looking for coloured pixels — the difference being that a lone dot of player-blue passes a pixel check and still yields a game with no player. **Both ways in use it**: `Draw.vue` on its canvas and `GameSetting.vue` on the photo, the latter on submit only, because a full pass over a phone photo is not something to do on every eyedropper click. The detection is shared; the wording is not — "you drew it too small" is wrong advice for a photograph, where the colour was picked out of the image itself.
...
`window.gamePaused` gates `update()`. The engine does not boot on import: `main.js` exports `bootGame()`, which resets the module-level state and returns the `Phaser.Game`; `Game.vue` calls it on mount (after setting the `window.*` inputs) and `game.destroy(true)` on unmount. The engine also reaches into the page by element id — `game-container`, `loading-screen`, `popup`, `popup-message`, `speedSlider`, `jumpSlider`, and `touch-left` / `touch-right` / `touch-jump` — so those ids in `Game.vue` are part of the contract. Input is arrows **or** WASD **or** those three buttons, which `Game.vue` renders only when `matchMedia('(pointer: coarse)')` matches; the check runs at setup rather than on mount because the engine binds them by id while it boots. Touch jump goes through `canJump` like the keys, and the listeners include `pointercancel`/`pointerleave` — without them a finger sliding off a button never releases it and the player runs forever. `canvas-confetti` is a CDN script injected by `Game.vue`; `showPopup` in `main.js` fires it via the global `confetti`.
...
- `FlashToast` listens two ways: the `flash` prop for anything the server flashed, and a `flash` **CustomEvent on `document`** for what it never saw — a refused save, a level gone before a request was made. Pages raise the event rather than writing into shared props, the same arrangement the engine uses for `level-won`.
- `/random-level` has its own path on purpose: `/play/random` would be caught by `/play/{drawing}`, whose model binding would read "random" as an id. It counts and offsets rather than `inRandomOrder()`, which sorts every published level to return one row, and an empty gallery is a message rather than a 404.
...
- **A username is 3–30 characters of letters, digits, hyphen and underscore**, and the rules live on `User` (`usernameRules()`, `USERNAME_MAX`) rather than in a form request — because **Google sign-in writes a username without passing through one**. `GoogleController::sanitisedUsername()` has to reproduce by hand what the rules promise: strip what is not allowed, `Str::ascii` first so accents become letters instead of vanishing, and fall back when nothing survives. Its de-duplication suffix must fit inside the limit too, so the base is trimmed to make room. Names already stored predate the rule, so the markup clamps as well — and a `truncate` inside a flex row needs `min-w-0` or it will not shrink.
- **Finishing a level is recorded.** `level_plays` is one row per person per drawing — `attempts` counts tries, `best_time_ms` is null until it is beaten, and that null is what separates "tried it" from "finished it" in every count ("beaten by 12 of 40", the leaderboard, your own best). Only an improvement is written, so replaying something you have beaten cannot cost you your best. The engine times the run with `performance.now()` (monotonic, so a clock change mid-run cannot produce a negative) and announces it with a `level-won` **CustomEvent on `document`** rather than another `window.*` global — the page listens, and the engine keeps not knowing that pages exist. **The clock runs in the browser, so a time is trusted input**: the request bounds it to 250 ms–1 hour, which only keeps out the impossible. Nothing server-side can verify a time while the game is client-side.
- **Keeping somebody else's level is favouriting it, not copying it.** `level_favourites` is one row per person per drawing (unique index, like `drawing_votes`) holding *your* `speed`/`jump_height` for their level. `play()` prefers those over the author's, so a kept level opens at your feel while theirs is untouched. Saving someone else's level through `/save-drawing` is now a **403** — it used to make you the owner of their drawing and let you republish it under your own name. Two things that break quietly here: the account page runs **two paginators**, so the favourites one needs its own page name (`'favouritesPage'`) or paging one silently pages the other; and a favourite outlives what it points at, because neither unpublishing nor a soft delete fires the foreign key cascade — the account query filters on `whereHas('drawing', published)` or the section renders cards that 404.
- **One picture is one file.** Level images are stored under the SHA-256 of their own contents (`storeByContent()`), so saving a level you *played* rather than made points at the file that is already there, and two people uploading the same picture write one file. The browser is never asked to upload an image the server already has. Copying used to be the rule, justified as letting the author's delete really remove their picture — it did not: with fifty copies it removed one of fifty identical files. Deleting a drawing soft-deletes the row and removes the file **only if no live row still names it**; that check in `destroy()` and `AccountController` is the mechanism, not a safety net, and it counts live rows only — trashed ones would pin a shared file forever. A trashed row naming a file that is gone is therefore normal. `levels:dedupe` (with `--dry-run`) is the one-off that collapsed the files written under the old rule. Keep `levels/` **flat**: sharding hash names into subdirectories would silently break `levels:prune`, which lists `files('levels')` and does not recurse.
